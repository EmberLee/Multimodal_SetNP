...
class LatentEncoder(nn.Module):
    ...
    """
    def __init__(self, num_hidden, num_latent, input_dim=args.dim_emr + 1, task_type='1d'):
        super(LatentEncoder, self).__init__()
        self.input_projection = Linear(input_dim, num_hidden)
        self.self_attentions = nn.ModuleList([Attention(num_hidden, task_type=task_type) for _ in range(2)])
...
class DeterministicEncoder(nn.Module):
    ...
    def __init__(self, num_hidden, num_latent, input_dim=args.dim_emr + 1, task_type='1d'):
        ...
        self.self_attentions = nn.ModuleList([Attention(num_hidden, task_type=task_type) for _ in range(2)])
        self.cross_attentions = nn.ModuleList([Attention(num_hidden, task_type=task_type) for _ in range(2)])
        self.input_projection = Linear(input_dim, num_hidden)
        self.context_projection = Linear(args.dim_emr, num_hidden)
        self.target_projection = Linear(args.dim_emr, num_hidden)
...
class Attention(nn.Module):
    ...
    Attention Network
    """
    def __init__(self, num_hidden, h=4, task_type='1d'):
        """
        ...
        self.layer_norm_double = nn.LayerNorm(num_hidden * 2)
    ...
    def forward(self, key, value, query):
        ...
        # Get context vector
        assert self.task_type in ['1d', '2d']
        result, attns = self.multihead(key, value, query)

        # Concatenate all multihead context vector
        result = result.view(self.h, batch_size, seq_q, self.num_hidden_per_attn)
        result = result.permute(1, 2, 0, 3).contiguous().view(batch_size, seq_q, -1) # -> (1,10,128)

        if self.task_type == '2d':
            # Concatenate context vector with input (most important)
            # example: x, y, target_x, target_y: (1,10,2), (1,10,1), (1,784,2), (1,784,1) by eric
            result = t.cat([residual, result], dim=-1)

            # Final linear
            result = self.layer_norm_double(result) # eric
